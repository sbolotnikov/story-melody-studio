...
export default function Dashboard() {
  const { user, profile, loading, updateProfile, resetPassword } = useAuth();
  ...
  if (loading || isLoadingData) {
    return <div className="flex-grow flex items-center justify-center p-24 text-muted-fg font-semibold uppercase tracking-widest text-xs">Loading...</div>;
  }
    ...
    return (
      <div className="flex-grow flex items-center justify-center py-24 px-4 bg-background">
        <div className="w-full max-w-md border border-border bg-muted p-8 shadow-2xl relative">
          <h2 className="text-2xl font-serif font-bold mb-4">Complete Profile</h2>
          <p className="text-sm text-muted-fg mb-6">
            We need your phone number to communicate with you about your customized gifts and questionnaires.
          </p>
          <form onSubmit={handleSavePhone} className="space-y-4">
            <div>
              <label className="block text-xs font-semibold uppercase tracking-widest text-muted-fg mb-2">Phone Number</label>
              <input
                type="tel"
                required
                value={phoneInput}
  ...
  return (
    <div className="flex-grow flex flex-col py-16 px-4 max-w-7xl mx-auto w-full">
        ...
        <section>
          ...
          <form className="max-w-2xl border border-border bg-background p-6 space-y-4" onSubmit={handleUpdateProfile}>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label className="block text-xs font-semibold uppercase tracking-widest text-muted-fg mb-2">Name</label>
                <input
                  type="text"
              ...
              <div>
                <label className="block text-xs font-semibold uppercase tracking-widest text-muted-fg mb-2">Phone</label>
                <input
                  type="tel"
            ...
            <div>
              <label className="block text-xs font-semibold uppercase tracking-widest text-muted-fg mb-2">Avatar URL</label>
              <input
                type="url"
            ...
            {profileMessage && (
              <p className="text-xs font-semibold uppercase tracking-widest text-green-600 mt-4">{profileMessage}</p>
            )}
          </form>
        </section>
                ...
                <div key={order.id} className="p-6 border border-border bg-background shadow-sm flex flex-col gap-6">
                  ...
                  <div className="flex-grow">
                    ...
                    <div className="text-sm text-foreground/80 mb-4 grid grid-cols-1 sm:grid-cols-2 gap-2 border-b border-border pb-4">
                      <div><strong>Date:</strong> {order.date || 'TBD'}</div>
                      <div><strong>Language:</strong> {order.language || 'English'}</div>
                      {profile?.role === 'admin' && (
                        <>
                           <div className="text-brand-gold"><strong>Email:</strong> {order.email || users.find(u => u.id === order.userId)?.email || 'N/A'}</div>
                           <div className="text-brand-gold"><strong>Phone:</strong> {order.phone || users.find(u => u.id === order.userId)?.phone || 'N/A'}</div>
                        </>
                      )}
                    </div>
                    {/* Summary for admin */}
                    {profile?.role === 'admin' && (
                      <div className="text-xs text-muted-fg space-y-1 mb-4">
                         {order.hero && <p><strong>Hero:</strong> {order.hero}</p>}
                         {order.story && <p><strong>Story:</strong> {order.story}</p>}
                      </div>
                    )}
                  </div>
                  
                  <div className="flex flex-col gap-3 min-w-[140px] border-t md:border-t-0 md:border-l border-border pt-4 md:pt-0 md:pl-6 justify-center">
                    {profile?.role === 'admin' && (
                      <select 
                    ...
                    {!order.paid && profile?.role !== 'admin' && (
                       <button
                         ...
                         className="text-xs font-bold uppercase tracking-[0.1em] bg-brand-gold text-brand-dark hover:bg-brand-gold/90 transition-colors py-3 px-4 text-center"
                       >
                         Pay Now
                       </button>
                    )}
                    ...
                    {!order.paid && profile?.role === 'admin' && (
                       <button
                         onClick={() => handlePayOrder(order.id)}
                         className="text-xs font-bold uppercase tracking-[0.1em] border border-green-500 text-green-500 hover:bg-green-500/10 transition-colors py-2 px-4 text-center"
                       >
                         Mark Paid
                       </button>
                    )}
                  ...
                  {expandedOrders[order.id] && (
                    <div className="border-t border-border pt-4 mt-2">
                       <h4 className="text-xs font-bold uppercase tracking-widest mb-4">Questionnaire Responses:</h4>
                ...
                <table className="w-full text-left text-sm">
                  <thead className="bg-muted/50 border-b border-border">
                    <tr>
                      ...
                      <th className="px-6 py-4 font-semibold uppercase tracking-widest text-xs">Phone</th>
                      <th className="px-6 py-4 font-semibold uppercase tracking-widest text-xs">Role</th>
                      <th className="px-6 py-4 font-semibold uppercase tracking-widest text-xs">Joined</th>
                      <th className="px-6 py-4 font-semibold uppercase tracking-widest text-xs">Orders</th>
                    </tr>
                  </thead>
                  <tbody>
                    {users.map(u => {
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                {products.map(prod => (
                  <div key={prod.id} className="p-6 border border-border bg-background shadow-sm flex flex-col">
                    ...
                    <p className="text-sm text-muted-fg mb-6 flex-grow">{prod.description}</p>
